...
export function InstanceDetailPage() {
  ...
  const activeTab = requestedTab === "tokens" || requestedTab === "profiles" ? "accounts" : requestedTab;
  ...
  const accountsSupported = !item.capabilities.length || item.capabilities.includes("accounts");
    ...
    <div className="page-stack">
      ...
      {activeTab === "overview" && <div className="page-stack">
        <section className="metric-band compact-metrics">
          ...
          <div><span>可用账号</span><strong>{snapshot ? `${accountStats?.available ?? snapshot.tokens.active}/${accountStats?.total ?? snapshot.tokens.total}` : "-"}</strong></div>
          <div><span>账号剩余积分</span><strong>{formatNumber(accountStats?.credits_available ?? snapshot?.tokens.credits_available, 1)}</strong></div>
        ...
        <section className="instance-facts"><div><span>应用版本</span><strong>{snapshot?.instance.version || "-"}</strong></div><div><span>构建 SHA</span><strong>{snapshot?.instance.build_sha || "-"}</strong></div><div><span>Cookie 账号</span><strong>{accountStats?.total ?? snapshot?.refresh_profiles.total ?? "-"}</strong></div><div><span>低积分账号</span><strong className={(accountStats?.low_credit || 0) > 0 ? "text-danger" : ""}>{accountStats?.low_credit ?? "-"}</strong></div><div><span>生成文件</span><strong>{formatNumber(snapshot?.storage.generated_file_count)}</strong></div><div><span>存储占用</span><strong>{snapshot ? `${snapshot.storage.generated_usage_mb} MB` : "-"}</strong></div></section>
      </div>}
      {compatible && activeTab === "accounts" && (accountsSupported ? <AccountsPage fixedInstanceId={instanceId} /> : <div className="warning-banner">当前实例端版本尚未提供 Cookie 账号接口，请先升级该实例。</div>)}
